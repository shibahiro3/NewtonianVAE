...
class MNVAECell(NewtonianVAECellBase):
    def __init__(
        self,
        dim_x: int,
        velocity: dict,
...
class LinearDynamicalCell(NewtonianVAECellBase):
    def __init__(
        self,
        dim_x: int,
        velocity: dict,
        ...
        self.f_velocity = component.Velocity(dim_x=dim_x, **velocity)
        self.p_transition = component.Transition5(dim_x=dim_x, **transition)
        self.q_encoder = component.MultiEncoder(dim_x=dim_x, **encoder)
        self.p_decoder = component.MultiDecoder(dim_x=dim_x, **decoder)
    ...
    def forward(self, I_t: List[Tensor], x_q_tn1: Tensor, u_tn1: Tensor):
        """"""
        ...
        # v_t = self.f_velocity(x_q_tn1, u_tn1, v_tn1, dt)
        # x_p_t = self.p_transition.given(x_q_tn1, v_t, dt).rsample()
        x_p_t = self.p_transition.given(x_q_tn1, u_tn1).rsample()
