...
def conv_nxn_bn(inp, oup, kernal_size=3, stride=1):
    return nn.Sequential(
        nn.Conv2d(inp, oup, kernal_size, stride, 1, bias=False), nn.BatchNorm2d(oup), nn.SiLU()
    )
...
class MobileViT(nn.Module):
    def __init__(
        self, image_size, dims, channels, num_classes, expansion=4, kernel_size=3, patch_size=(2, 2)
    ):
        """maybe only 2^n img_size"""
        super().__init__()
...
class MobileViTWrap(MobileViT):
    def __init__(
        ...
        assert param_version in ("xxs", "xs", "s")
        if param_version == "xxs":
            dims = [64, 80, 96]
            channels = [16, 16, 24, 24, 48, 48, 64, 64, 80, 80, 320]
        elif param_version == "xs":
            dims = [96, 120, 144]
            channels = [16, 32, 48, 48, 64, 64, 80, 80, 96, 96, 384]
        elif param_version == "s":
            dims = [144, 192, 240]
            channels = [16, 32, 64, 64, 96, 96, 128, 128, 160, 160, 640]

        super().__init__(img_size, dims, channels, dim_output, expansion, kernel_size, patch_size)
