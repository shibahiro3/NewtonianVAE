...
class EnvBase:
    # no torch on observation
    # ObservationItemType = np.ndarray
    ObservationItemType = Union[np.ndarray, Real]
    ObservationType = Dict[str, Union[ObservationItemType, Dict[str, ObservationItemType]]]

    def reset() -> ObservationType:
        raise NotImplementedError()

    def step(self, action: Tensor) -> Tuple[ObservationType, bool]:
        raise NotImplementedError()


class ControlSuite(EnvBase):
    def __init__(
        self,
        env: str,
        seed: int,
        ...
        assert imgsize[0] <= 480
        assert imgsize[1] <= 640
        ...
        # Unused in self (for just reference)
        ...
        domain, task = env.split("-")
        task_kwargs = {"random": seed, "task_settings": task_settings, "xml_file": xml_file}
        self._env: control.Environment = suite.load(
    ...
    def reset(self) -> EnvBase.ObservationType:
        self.t = 0  # Reset internal timer
        state = self._env.reset()
        return self._observations(state)

    def step(self, action: Tensor) -> Tuple[EnvBase.ObservationType, bool]:
    ...
    def render(self):
        ...
        mv.show_imgs_cv(
            list(cameras.values()), winname, cols=-1, color_order="RGB", block=False, delay=1
        )
    ...
    def _cameras(self) -> Dict[str, np.ndarray]:
        ...
        for name in self._env.physics.named.model.cam_quat.axes.row.names:
            cameras[name] = self._env.physics.render(
                height=self.imgsize[0], width=self.imgsize[1], camera_id=name
            )
        return cameras

    def _observations(self, state) -> EnvBase.ObservationType:
        observations = state.observation
        observations["camera"] = {}
        for k, v in self._cameras().items():
            observations["camera"][k] = v
        return observations
...
#         # https://github.com/deepmind/dm_control/issues/116
#         for name in self._env.physics.named.model.cam_quat.axes.row.names:
#             # max height and width: 480
#             cameras[name] = self._env.physics.render(
#                 height=self.imgsize[0], width=self.imgsize[1], camera_id=name
#             )
#         return cameras

#     ObservationType = Dict[str, Union[np.ndarray, Dict[str, np.ndarray]]]

#     def _observations(self, state) -> ObservationType:
#         observations = state.observation
#         observations["camera"] = {}
#         for k, v in self._cameras().items():
...
#     def _observations_raw(self, state) -> ObservationType:
#         observations = state.observation
#         observations["camera"] = {}
#         for k, v in self._cameras().items():
#             observations["camera"][k] = v
...
#     def step(
#         self, action: Tensor, raw=False
#     ) -> Tuple[Dict[str, Union[np.ndarray, Tensor, Real]], bool]:
#         """
...
#         if raw:
#             return self._observations_raw(state), done
#         else:
#             return self._observations(state), done
