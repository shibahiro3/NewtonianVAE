...
class ABCf(nn.Module):
    ...
    def forward(self, x_tn1, v_tn1, u_tn1):
        """
        Returns:
        ...
        # return diagA, -log_diagnB.relu(), log_diagC.relu()
...
class Encoder(tp.Normal):
    ...
    def __init__(self, dim_x: int, dim_middle=512) -> None:
        ...
        self.std = nn.Linear(dim_middle, dim_x)  # transition prior の固定 std に合うようになるだけ..?
...
class Decoder(tp.Normal):
    ...
    このstdは係数が変わるだけなので、lossは変わるが学習には無意味
    NLL が大きく変わる
    """
