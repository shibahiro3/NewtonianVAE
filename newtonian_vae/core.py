...
class NewtonianVAECell(NewtonianVAECellBase):
    ...
    def forward(self, I_t: Tensor, x_tn1: Tensor, u_tn1: Tensor, v_tn1: Tensor, dt: float):
        ...
        if self.training:
            ...
            E = E_ll - E_kl  # やっぱり + E_kl にするとおかしくなる
            ...
            # Paper: During inference, vt is computed as vt = (xt − xt−1)/∆t, with xt ∼ q(xt|It) and xt−1 ∼ q(xt−1|It−1).
...
class NewtonianVAECellDerivation(NewtonianVAECellBase):
    def __init__(
        self,
        dim_x: int,
        dim_xhat: int,
...
class Stepper:
    def __init__(self, cell: Union[NewtonianVAECell, NewtonianVAECellDerivation]) -> None:
        self.cell = cell
...
class CollectTimeSeriesData:
    def __init__(self, dim_x, dim_xhat, T, dtype: np.dtype) -> None:
        ...
        self.LOG_x = xp.full((T, dim_x), xp.nan, dtype=dtype)
        self.LOG_x_mean = xp.full((T, dim_x), xp.nan, dtype=dtype)
        self.LOG_v = xp.full((T, dim_x), xp.nan, dtype=dtype)
        self.LOG_xhat_mean = xp.full((T, dim_xhat), xp.nan, dtype=dtype)
        self.LOG_xhat_std = xp.full((T, dim_xhat), xp.nan, dtype=dtype)
    ...
    def run(
        ...
        for t in range(T):
            ...
            if t == 0:
                ...
                x_t = cell.q_encoder.cond(I_t).rsample()
            else:
                E, E_ll, E_kl, x_t, v_t = cell(I_t, x_tn1, u_tn1, v_tn1, 0.1)
                ...
                if is_save:
                    self.LOG_I_dec[t] = self.as_save(cell.p_decoder.decode())
                    if type(cell) == NewtonianVAECellDerivation:
                        self.LOG_xhat_std[t] = self.as_save(cell.p_xhat.scale)
                        self.LOG_xhat_mean[t] = self.as_save(cell.p_xhat.loc)
            ...
            if is_save:
                ...
                self.LOG_x_mean[t] = self.as_save(cell.q_encoder.loc)
